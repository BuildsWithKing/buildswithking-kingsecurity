...
abstract contract KingAccessControlLite {
    ...
    event RoleGranted(address king, bytes32 indexed role, address indexed account);
    ...
    event RoleRevoked(address king, bytes32 indexed role, address indexed account);
    ...
    /// @param role The role required for execution.
    ...
    /// @notice Restrict access to only the king.
    modifier onlyKing() {
    ...
    function transferKingRole(address newKing) external onlyKing {
        ...
        // Return if old king's address is the same as the new king's address.
